...
function getSiHuas(life) {
    ...
    for (let palaceFrom of life.palaces) {
        ...
        bingLianMap.shengnianshuangxiang[palaceFrom.name] = []
        // 遍历每宫星耀
        for (let star of starFroms) {
            // 生年四化
            if (star.mutagen) {
                shengNianSiHuas.push([palaceFrom.name, star.name, star.mutagen])
                bingLianMap.shengnianshuangxiang[palaceFrom.name].push([palaceFrom.name, star.name, star.mutagen])
            }
            // 离心四化
        ...
        for (let star of starTos) {
            // 生年四化
            if (star.mutagen) {
                bingLianMap.shengnianshuangxiang[palaceFrom.name].push([palaceTo.name, star.name, star.mutagen])
            }
            // 向心四化
            const starSiHua = ziwei.SIHUAMAP[palaceFrom.heavenlyStem][star.name]
            if (starSiHua) {
    ...
    for (let gong in bingLianMap.shengnianshuangxiang) {
        // 删除重复对宫
        delete bingLianMap.shengnianshuangxiang[ziwei.DUIGONGMAP[gong]]
        // 删除非双象
        if (bingLianMap.shengnianshuangxiang[gong].length < 2) {
            delete bingLianMap.shengnianshuangxiang[gong]
        }
    }
